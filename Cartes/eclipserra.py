...
pokemon_image = pygame.image.load("Eclipserra.jpg")
pokemon_image = pygame.transform.scale(pokemon_image, (300, 300))  # Redimensionner l'image

# Données de la carte Pokémon
pokemon_nom = "Eclipserra"
pokemon_type = "Psy/Ténèbres"
pokemon_pv = 90
attaque_1 = "Onde Nocturne"
atk_1_degats = 50
description = "Desciption : Mi-ombre, mi-lumière, Eclipserra représente l’équilibre entre le bien et le mal. Son corps est marqué par des motifs évoquant des éclipses lunaires."
...
def draw_card():
    ...
    pygame.draw.rect(screen, BLANC, (10, 10, largeur - 20, hauteur - 20), 5)

    # Dessiner le nom du Pokémon
    title_text = font_title.render(pokemon_nom, True, BLANC)
    screen.blit(title_text, (250, 30))

    # Dessiner le type du Pokémon
    type_text = font_subtitle.render(f"Type: {pokemon_type}", True, BLANC)
    screen.blit(type_text, (30, 80))

    # Dessiner les PV
    pv_text = font_subtitle.render(f"PV: {pokemon_pv}", True, BLANC)
    ...
    attaque_1_text = font_text.render(f"Attaque 1: {attaque_1} ({atk_1_degats} dégats)", True, BLANC)
    ...
    pygame.draw.rect(screen, BLANC, (30, 150, 200, 30), 3)  # Contour de la barre de vie
    
    #Dessiner la description
    type_text = font_text.render(description, True, BLANC)
